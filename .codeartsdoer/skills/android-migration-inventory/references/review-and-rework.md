...
Only the coverage checker may issue the final closure decision. It verifies:
...
The evidence administrator validates identity and integrity; the coverage checker decides sufficiency.
...
Final decisions are `PASS`, `INCOMPLETE`, or `BLOCKED`. `PASS` requires zero open reworks of any severity and zero pending confirmations.
