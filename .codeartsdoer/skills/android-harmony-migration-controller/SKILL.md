...
- Frozen values are written to scope.json (`screen_resolution/screen_density/serial` + `sdk_toolchain`); P2 gmi_runtime (`--screen-size/--screen-density`, falls back to scope frozen values), P3/P4 builds and H4ENV reuse them directly, never changing the baseline.

**GMI-mode Gate 2/3/4 rules** (controller machine recheck for GMI workflows):
- Detection: `phase-02-android-inventory/gmi/phase-2-closure.json` exists (or phase-manifest `generator: gmi`) means a GMI run. Gate 2 recomputes the 13 candidate tables, closure, UNMAPPED and audit; Gate 3 adds architecture mapping and stage-03 seal; Gate 4 adds per-page contracts, behavior bindings, non-empty components and stage-04 seal. Gate 3 never substitutes for Gate 4. In CodeArts runs, Gate 4 must also independently recompute per-page `ACCEPTED` implementation coverage, canonical ArkTS paths, placeholder-shell rejection, parity coverage, sealed emulator/UiTest packages, acceptance records, screenshot uniqueness and the complete closure manifest; never trust a PASS report alone.
- Work-order issuance: only `issue_phase3_work_order.py` / `issue_phase4_work_order.py`; they recompute the GMI Gate and generate uppercase IDs. Never hand-edit gate-report or hand-assemble work orders.
- Adapter no-clobber: `gmi_phase3_adapter.py --out` pointing at an existing run keeps the controller identity and real static-analysis, aligning old Page-IDs to GMI Page-IDs; when components are missing, synthesize them only deterministically from accepted UI trees or page-fields.
