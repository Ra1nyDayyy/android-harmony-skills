...
Only the architecture lead ID frozen in the Phase 3 work order (carried by the gmi Phase 2 closure handoff) may create and freeze a HENV. `freeze_environment.py` rejects a `CLOSED` Phase 3 workspace and writes the environment JSON read-only after hashing it.
...
**Screen parity with Phase 2 (required):** the baseline device's `resolution` and `density` must be **identical** to the frozen Android Phase 2 evidence device used for the same Page-IDs (gmi `runtime-evidence/evidence-index.csv` resolution/density per evidence row). Phase 3 initialization rejects a baseline whose resolution/density differs from the Android evidence; any mismatch is `BLOCKED - screen-parity` and must be fixed by re-fixing the emulator, never by rescaling screenshots.
