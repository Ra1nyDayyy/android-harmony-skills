...
Consume the controller-issued Phase 3 work order (legacy) or **gmi Phase 2 closure** (default path: `phase-2-closure.json` + `candidates/` 12 表 + `runtime-evidence/` + `audit-replay.csv` — see [input-mapping-contract.md](references/input-mapping-contract.md) gmi section). Run `scripts/init_scaffold.py`; it copies the accepted inputs, creates the template project, seeds architecture, page/surface, asset-landing, capability, and migration-status registries, and rejects input drift.
